...
# ---------------------------------------
# 🗺️ Folium Map - Exclusive Base Layers + Custom Color Palettes
# ---------------------------------------
def get_colormap(df, column_name, palette_name):
    values = df[column_name]
    min_val = round(values.min())
    max_val = round(values.max())
    if min_val == max_val:
        min_val = 0
    palette = getattr(cm.linear, palette_name).to_step(10)
    return cm.LinearColormap(palette.colors, vmin=min_val, vmax=max_val).to_step(10)

def create_layer(gdf, column, name, palette):
    cmap = get_colormap(gdf, column, palette)
    style_fn = lambda x: {
        "weight": 0.5,
        'color': 'black',
...
# Base (exclusive) layers with unique color palettes
fg_30 = folium.FeatureGroup(name='30-Day Utilization', overlay=False)
fg_60 = folium.FeatureGroup(name='60-Day Utilization', overlay=False)
fg_90 = folium.FeatureGroup(name='90-Day Utilization', overlay=False)

fg_30.add_child(create_layer(gdf_afr1, '30_day_utilization', '30-Day Utilization', 'YlGnBu_09'))
fg_60.add_child(create_layer(gdf_afr1, '60_day_utilization', '60-Day Utilization', 'PuRd_09'))
fg_90.add_child(create_layer(gdf_afr1, '90_day_utilization', '90-Day Utilization', 'YlOrRd_09'))

map_afr1.add_child(fg_30)  # default active
map_afr1.add_child(fg_60)
map_afr1.add_child(fg_90)
